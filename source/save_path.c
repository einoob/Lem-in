...
/*   Updated: 2020/07/27 20:33:03 by elindber         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		enough_paths(t_info *info, int path_index)
{
	...
	)//|| (info->phase == 2 && info->last_found_used_2 == 0))
		return (1);
...
//		return ;
	...
	&& (!(info->rooms[info->index_stack[path_i][0]]->ant_queue = (int*)malloc(sizeof(int) * info->ants + 1))
...
//		ft_printf("\nS A V I N G (%d round):\n[%s]\n", info->round, info->valid_paths_2[info->path_amount_2]);
